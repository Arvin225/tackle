...
export function useSync(webDAVService: WebDAVService) {
   ...
   * Enable/disable sync
   */
  const setSyncEnabled = useCallback((enabled: boolean): void => {
    if (enabled) {
      syncManager.enable()
    } else {
      syncManager.disable()
   ...
   * Set auto sync
   */
  const setAutoSync = useCallback((autoSync: boolean): void => {
   ...
   * Set sync interval
   */
  const setSyncInterval = useCallback((intervalMs: number): void => {
